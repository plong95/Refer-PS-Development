...
public final class TextRSGraphics extends RSGraphics {

	public TextRSGraphics(boolean flag, String s, CacheArchive streamLoader) {
		try {
	...
	public void method38(String s, int i, int j, int colour, boolean flag)
	{
		...
		int i1 = method44();
		if(i - l > RSGraphics.bottomY)
			return;
		if(i + l < RSGraphics.topX)
			return;
		if(j - i1 > RSGraphics.bottomX)
			return;
		if(j < 0)
		{
			return;
	...
	private void drawChar(byte text[], int x, int y, int width, int height, int color, int opacity) {
		int newOffset = x + y * RSGraphics.width;
		int place1 = RSGraphics.width - width;
		int place2 = 0;
		int oldOffset = 0;
		if (y < RSGraphics.topY) {
			int Height2 = RSGraphics.topY - y;
			height -= Height2;
			y = RSGraphics.topY;
			oldOffset += Height2 * width;
			newOffset += Height2 * RSGraphics.width;
		}
		if (y + height >= RSGraphics.bottomY) {
			height -= ((y + height) - RSGraphics.bottomY);
		}
		if (x < RSGraphics.topX) {
			int toLeft = RSGraphics.topX - x;
			width -= toLeft;
			x = RSGraphics.topX;
			oldOffset += toLeft;
			newOffset += toLeft;
		...
		if (x + width >= RSGraphics.bottomX) {
			int toRight = ((x + width) - RSGraphics.bottomX);
			width -= toRight;
			place2 += toRight;
			place1 += toRight;
		}
		...
		setPixelsInfo(width, height, opacity, RSGraphics.pixels, text, newOffset, oldOffset, place1, place2, color);
	}
	...
	public void drawRegularText(boolean flag1, int i, int j, String s, int k) {
		...
		if (aBoolean1499)
			RSGraphics.drawLine(k + (int) ((double) anInt1497 * 0.69999999999999996D), 0x800000, i - l, l);
	}
	...
	private void method392(byte abyte0[], int i, int j, int k, int l, int i1) {
		int j1 = i + j * RSGraphics.width;
		int k1 = RSGraphics.width - k;
		int l1 = 0;
		int i2 = 0;
		if (j < RSGraphics.topY) {
			int j2 = RSGraphics.topY - j;
			l -= j2;
			j = RSGraphics.topY;
			i2 += j2 * k;
			j1 += j2 * RSGraphics.width;
		}
		if (j + l >= RSGraphics.bottomY)
			l -= ((j + l) - RSGraphics.bottomY) + 1;
		if (i < RSGraphics.topX) {
			int k2 = RSGraphics.topX - i;
			k -= k2;
			i = RSGraphics.topX;
			i2 += k2;
			j1 += k2;
		...
		if (i + k >= RSGraphics.bottomX) {
			int l2 = ((i + k) - RSGraphics.bottomX) + 1;
			k -= l2;
			l1 += l2;
			k1 += l2;
		}
		if (!(k <= 0 || l <= 0)) {
			method393(RSGraphics.pixels, abyte0, i1, i2, j1, k, l, k1, l1);
		}
	}
	...
	private void method394(int i, int j, byte abyte0[], int k, int l, int i1, int j1) {
		int k1 = j + l * RSGraphics.width;
		int l1 = RSGraphics.width - k;
		int i2 = 0;
		int j2 = 0;
		if (l < RSGraphics.topY) {
			int k2 = RSGraphics.topY - l;
			i1 -= k2;
			l = RSGraphics.topY;
			j2 += k2 * k;
			k1 += k2 * RSGraphics.width;
		}
		if (l + i1 >= RSGraphics.bottomY)
			i1 -= ((l + i1) - RSGraphics.bottomY) + 1;
		if (j < RSGraphics.topX) {
			int l2 = RSGraphics.topX - j;
			k -= l2;
			j = RSGraphics.topX;
			j2 += l2;
			k1 += l2;
		...
		if (j + k >= RSGraphics.bottomX) {
			int i3 = ((j + k) - RSGraphics.bottomX) + 1;
			k -= i3;
			i2 += i3;
			l1 += i3;
		}
		if (k <= 0 || i1 <= 0)
			return;
		method395(abyte0, i1, k1, RSGraphics.pixels, j2, k, i2, l1, j1, i);
	}
