...
public class RSGraphics extends QueueNode {
	...
	public static void setDrawingArea(int yBottom, int xTop, int xBottom, int yTop)
	{
		...
		bottomY = yBottom;
		lastX = bottomX - 0;
		clipCenterX = bottomX / 2;
		clipCenterY = bottomY / 2;
	}
    ...
    public static void drawStylishBox(int x, int y, int width, int height) {
    	RSGraphics.drawBoxOutline(x, y, width, height, STYLISH_BOX_OUTLINE_OUTLINE_COLOR);
    	RSGraphics.drawBoxOutline(x + 1, y + 1, width - 2, height - 2, STYLISH_BOX_OUTLINE_COLOR);
        RSGraphics.drawTransparentBox(x + 2, y + 2, width - 4, height - 4, STYLISH_BOX_BACKGROUND_COLOR, 156);
    }
	...
	public static void drawBox(int topX, int topY, int width, int height, int rgbColour) {
		if (topX < RSGraphics.topX) {
			width -= RSGraphics.topX - topX;
			topX = RSGraphics.topX;
		}
		if (topY < RSGraphics.topY) {
			height -= RSGraphics.topY - topY;
			topY = RSGraphics.topY;
		}
		if (topX + width > bottomX)
			width = bottomX - topX;
		if (topY + height > bottomY)
			height = bottomY - topY;
		int leftOver = RSGraphics.width - width;
		int pixelIndex = topX + topY * RSGraphics.width;
		for (int rowIndex = 0; rowIndex < height; rowIndex++) {
