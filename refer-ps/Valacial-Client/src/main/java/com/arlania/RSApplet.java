...
WindowListener {
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		if (mouseWidth > 0 && mouseWidth < 512
				&& mouseHeight > Client.getClient().clientHeight - 165
				&& mouseHeight < Client.getClient().clientHeight - 25) {
			...
			boolean zoom = Client.clientSize == 0 ? (mouseWidth < 512) : (mouseWidth < Client.clientWidth - 200);
			if(zoom && Client.openInterfaceID > 0) {
			}else {
	...
	public boolean handleInterfaceScrolling(MouseWheelEvent event) {
		...
		if (tabInterfaceID != -1) {
			...
			if (mouseWidth > offsetX + positionX && mouseHeight > offsetY + positionY
					&& mouseWidth < offsetX + positionX + width
					&& mouseHeight < offsetY + positionY + height) {
