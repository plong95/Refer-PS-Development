...
import com.arlania.CacheArchive;
import com.arlania.FileOperations;
import com.arlania.Sprite;
...
public class SpriteLoader {
	...
	public static void createSprite(SpriteLoader sprite, boolean second) {
		File directory = new File(signlink.findcachedir() + "sprites");
		if (Configuration.dumpSprites) {
			if (!directory.exists()) {
				directory.mkdir();
			}
			FileOperations.WriteFile(directory.getAbsolutePath()
					+ System.getProperty("file.separator")
					+ sprite.id + ".png", sprite.spriteData);
			System.out.print("Successfully dumped [" + sprite.id + "] sprites.\n");
		}
		if (!second) {
