...
public class RSInterface {
	...
	public static final int WRAPPED_TEXT = 19; //type
    ...
    public static void addHoverImage2(int id, int spriteOn, int spriteOff, int width, int height, int configFrame) {
        RSInterface tab = addTabInterface(id);
        tab.id = id;
        tab.type = 5;
        tab.hidden = true;
	...
	public static RSInterface addWrappingText(int id, String text, TextDrawingArea tda[],
		...
		RSInterface tab = addWrappingText(id, text, tda, idx, color, center, shadow, width, tda[idx].textLineHeight);
		return tab;
	}
    ...
    public static void itemGroupAutoScroll(int id, int itemsId, int w, int h, int x, int y, int viewableColumns) {
        ...
        int scrollItemAmount = w - viewableColumns;
    ...
    public static void addButtonWithHover(int id, int sid, String tooltip, int w, int h, int hoverId) {
        ...
        tab.height = h;
        tab.tooltip = tooltip;
        interfaceCache[id].hoverType = hoverId;
    }

	public static void addSingleHoverSprite(int id, int sprite, int width, int height) {
		RSInterface tab = addTabInterface(id);
		tab.id = id;
		tab.type = 5;
		tab.hidden = true;
	...
	public static void addConfigButtonWSpriteLoaderWithHover(int ID, int pID, int bID, int bID2, int width, int height,
															 String tT, int configID, int aT, int configFrame, int hoverId) {
...
//		RSInterface.addText(35002, "Instance Manager", tda, 2, ColorConstants.YELLOW, false, true);
//		RSInterface.addButton1(35003, 1245, 1246, "Select");
//		RSInterface.drawNpcOnInterface(35004, 100, 1000);
//		RSInterface.addText(35005, "Start Instance", tda, 2, ColorConstants.YELLOW, false, true);
...
//			RSInterface.addText(35071 + i, "boss", tda, 1, ColorConstants.ORANGE
//			);
//
//		}
...
public static void zoneTasksInterface(TextDrawingArea[] tda) {
	...
	for (int i = 0; i < 30; i++) {
		addButtonWSpriteLoader(19960 + i, 1350, "Claim Reward");
	...
	private static void shopInterface() {
		...
		addSpriteLoader(3825, 1295);
		addHoverButtonWSpriteLoader(78751, 1296, 21, 21, "Close Window", 0, 78752, 1);
		addHoveredImageWSpriteLoader(78752, 1297, 21, 21, 78753);
		addText(78754, "ImaginePS Stores", fonts, 2, ORANGE_TEXT, true, true);
		addText(78755, "Available Stores", fonts, 3, 0xff9040, true, true);
		addText(45612, "", fonts, 1, ORANGE_TEXT, true, true);
		addText(78757, "Shop Currency:", fonts, 0, 0xff9040, false, true);
		addText(12315, "", fonts, 0, ORANGE_TEXT, false, true);
		addToggleButton(78759, 1325, 4818, 98, 16, "Toggle");
		addText(78760, "Send to bank?:", fonts, 0, 0xff9040, false, true);
		setBounds(3825, 7, 12, 0, rsinterface);
		setBounds(78751, 472, 21, 1, rsinterface);
		setBounds(78752, 472, 21, 2, rsinterface);
		setBounds(78754, 260, 23, 3, rsinterface);
		setBounds(78755, 94, 56, 4, rsinterface);
		setBounds(45612, 335, 56, 5, rsinterface);
		setBounds(78757, 305, 294, 6, rsinterface);
		setBounds(12315, 387, 294, 7, rsinterface);
		setBounds(62300, -153, 87, 8, rsinterface);
		setBounds(29995, 187, 87, 9, rsinterface);
		setBounds(78759, 185, 290, 10, rsinterface);
		setBounds(78760, 190, 294, 11, rsinterface);
		addHoverButtonWSpriteLoader(78001, 1298, 46, 50, "Value", 0, 78002, 1);
		addHoveredImageWSpriteLoader(78002, 1299, 46, 50, 78003);

		rsinterface = interfaceCache[3900];
		setChildren(3, rsinterface);
		setBounds(78001, 6, 1, 0, rsinterface);
		setBounds(78002, 6, 1, 1, rsinterface);

		rsinterface.inv = new int[5000];
		rsinterface.invStackSizes = new int[5000];
		rsinterface.drawInfinity = true;
		rsinterface.invSpritePadX = 26;
		rsinterface.width = 5;
		rsinterface.height = 500;
		rsinterface.invSpritePadY = 25;

		RSInterface scrollInterface = addTabInterface(62300);
		scrollInterface.width = 307;
		scrollInterface.height = 222;
		scrollInterface.scrollMax = (950 * 2) * 2;
		setChildren(300 * 2, scrollInterface);
		...
		for (int i = 0; i < 200; i++) {
			...
			int slot = 32156 + i + (5 * 1);
			addText(slot, "", fonts, 0, ORANGE_TEXT, true, true);
			setBounds(slot, 237, y + 3, frame, scrollInterface);
			frame++;
			addConfigButtonWSpriteLoader(52756, 3824, 1247, 1246, 132, 18, "Select Store", id + 1, 5, 507);
//			addConfigButtonWSpriteLoader(id, 3824, 1247, 1246, 132, 18, "Select Store", i, 5, 507, id + 1, 25);
			setBounds(id, 171, y, frame, scrollInterface);
			frame++;

			addHoveredImageWSpriteLoaderTrans(id + 1, 1300, 132, 18, id + 2, 25);
			setBounds(id + 1, 171, y, frame, scrollInterface);
			frame++;
			id += 3;
		...
		for (int xy = 0; xy < 31; xy++) {
			...
			ys = (39 + xy) + (56 * xy);
			for (int i = 0; i < 5; i++) {
				setBounds(35613 + i + (5 * xy), x, ys, 1 + i + (5 * xy), scroll);
				x += 58;
			}
		}
		...
		scroll.height = 195;
		scroll.width = 289;
		scroll.scrollMax = 1214;

		// Position the scroll bar interface (containing the container) in the
		// actual shop interface

	}
	public static void collectionLog(TextDrawingArea[] tda) {
		RSInterface tab = addInterface(63000);
		String dir = "Interfaces/CollectionLog/SPRITE";
		addSprite(63001, 0, dir);
		addText(63002, "Collection Log", tda, 2, 0xff981f, true, true);
		...
		for (int i = 0; i < tabNames.length; ++i) {
			...
			addConfigButton(63003 + i, 63000, 1, 2, dir, 96, 20, "View " + tabNames[i], i, 5, 1106);
			addText(63008 + i, tabNames[i], tda, 1, 0xff981f, false, true);
		}

		addText(63015, "", tda, 2, 0xff981f, false, true);
		addText(63016, "", tda, 0, 0xff981f, false, true);
		addText(63017, "", tda, 0, 0xff981f, false, true);
		interfaceCache[63017].rightText = true;

		int x = 10, y = 10, child = 0;
		tab.totalChildren(16);
		tab.child(child++, 63001, x, y);
		tab.child(child++, 63002, 250+x, 4+y);
		for (int i = 0; i < tabNames.length; ++i) {
			if (i == 2) {
				continue;
			}
			if (i == 3) {
				tab.child(child++, 63003 + i, 10+x+(2 * 96), 24+y);
				tab.child(child++, 63008 + i, 14+x+(2 * 96), 27+y);
				continue;
			}

			if (i == 4) {
				tab.child(child++, 63003 + i, 10+x+(3 * 96), 24+y);
				tab.child(child++, 63008 + i, 14+x+(3 * 96), 27+y);
				continue;
			}

			tab.child(child++, 63003 + i, 10+x+(i * 96), 24+y);
			tab.child(child++, 63008 + i, 14+x+(i * 96), 27+y);
		}
		tab.child(child++, 63025, 214+x, 84+y);
		tab.child(child++, 63050, 11+x, 45+y);
		tab.child(child++, 63015, 216+x, 47+y);
		tab.child(child++, 63016, 216+x, 71+y);
		tab.child(child++, 63017, 484+x, 71+y);

		RSInterface items = addInterface(63025);
		addToItemGroup(63026, 6, 20, 12, 6, false, null, null, null);

		items.totalChildren(1);
		items.child(0, 63026, 5, 5);
		items.width = 259;
		items.height = 222;
		items.scrollMax = 800;

		RSInterface scroll = addInterface(63050);
		scroll.totalChildren(150);
		for (int i = 0; i < 50; ++i) {
			addHoverButton(63051 + i, dir, i % 2 == 0 ? 3 : 5, 186, 15, "View", -1, 63101 + i, 1);
			addHoveredButton(63101 + i, dir, 4, 186, 15, 63049);
			addText(63151 + i, "", tda, 1, 0xff981f, false, true);
			scroll.child(i, 63051 + i, 0, i * 15);
			scroll.child(i + 50, 63101 + i, 0, i * 15);
			scroll.child(i + 100, 63151 + i, 2, i * 15);
		}
		scroll.width = 186;
		scroll.height = 261;
		...
		tab.child(child++, 35255, 486, 13);
	}

	public static void dailyReward(TextDrawingArea[] tda) {
		RSInterface tab = addTabInterface(58000);
		int child = 58001;

		addSprite(child++, 0, "Interfaces/DailyReward2/SPRITE");
		...
		addText(child++, "31 Days of Rewards", tda, 2, 0xff7000, false); // Right side header

		addText(child++, "Sentence 1", tda, 0, 0xff7000, false);
		addText(child++, "", tda, 0, 0xff7000, false);
		addText(child++, "", tda, 0, 0xff7000, false);

		addButton(child++, 4, "Interfaces/DailyReward2/SPRITE", "Claim");
		...
		for (int i = 0; i < 31; i++) {
			addSprite(58101 + i, 1, "Interfaces/DailyReward2/SPRITE");
		}
	...
	public static void teleportInterfaceNew(TextDrawingArea[] tda) {
		...
		addText(45803, "Bosses", tda, 2, 0xffbf59, false, true);
		addText(45804, "Champion", tda, 2, 0xffbf59, false, true);
		addText(45805, "Drops", tda, 2, 0xffbf59, false, true);
		addText(45806, "Information", tda, 2, 0xffbf59, false, true);
		addText(45807, "Attack with:", tda, 1, 0xed9507, false, true);
		addText(45808, "Hp:", tda, 1, 0xed9507, false, true);
		addText(45809, "Maxhit:", tda, 1, 0xed9507, false, true);
		addText(45810, "tier:", tda, 1, 0xffbf59, true, true);
		addText(45843, "Teleport", tda, 1, 0xed9507, false, true);

		addText(45836, "Monsters", tda, 2, 0xed9507, false, true);
		addText(45837, "Bosses", tda, 2, 0xed9507, false, true);
		addText(45838, "Minigames", tda, 2, 0xed9507, false, true);
		addText(45839, "Global", tda, 2, 0xed9507, false, true);
		addText(45935, "Ultra Bosses", tda, 2, 0xed9507, false, true);

		addText(45840, "Maxhit", tda, 1, 0xed9507, false, true);
		addText(45841, "Players here", tda, 1, 0xed9507, false, true);
		addText(45934, "@yel@Teleports", tda, 2, 0xed9507, true, true);
		...
		RSInterface scroll = addInterface(45900);
		...
		for (int i = 0; i < 15; i++) {
			addHoverButtonWSpriteLoader(childId++, 1244, 151, 20, "Add", -1, -1, 1);
			addHoverButtonWSpriteLoader(childId++, 1245, 151, 20, "Add", -1, -1, 1);
		...
		y = 0;
		for (int j = 0; j < 30; j++) {
			scroll.child(j, 45901 + j, 5, 3 + y);
			y += 20;
		}
		y = 5;
		for (int j = 30; j < 60; j++) {
			scroll.child(j, 45932 + j, 10, y);
			y += 20;
		}


		RSInterface scroll2 = addInterface(45500);
		scroll2.scrollMax = 500;
		scroll2.height = 113;
		scroll2.width = 120;

		addToItemGroup(45501, 3, 30, 8, 8, false, null, null, null);
	...
	private static void questTabInterface() {
		...
		addSpriteLoader(26603, 1327);
		addSpriteLoader(26620, 1265);
		addSpriteLoader(26604, 722);

		addText(26608, "Players Online: @gre@30", fonts, 2, 16750623, false, true);

		addText(26609, "www.ImaginePS.org", fonts, 0, 16750623, false, true);

		addHoverButtonWSpriteLoader(26610, 1331, 21, 18, "", -1, 26611, 1);
		addHoveredImageWSpriteLoader(26611, 1337, 21, 18, 26612);

		addHoverButtonWSpriteLoader(26613, 1333, 21, 18, "", -1, 26614, 1);
		addHoveredImageWSpriteLoader(26614, 1336, 21, 18, 26615);

		addHoverButtonWSpriteLoader(26616, 1328, 21, 18, "", -1, 26617, 1);
		addHoveredImageWSpriteLoader(26617, 1338, 21, 18, 26618);

		addText(26619, "Character Summary", fonts, 2, 16750623, false, true);
		addText(37463, "Game rank: Player", fonts, 1, 16750623, false, false);
		addText(37464, "127", fonts, 1, 16750623, false, true);
		addText(37465, "2016", fonts, 1, 16750623, false, true);
		addText(37466, "127.000.000.000", fonts, 0, 16750623, false, true);

		addText(37467, "Droprate", fonts, 0, 16750623, false, true);
		addText(37468, "Double Droprate", fonts, 0, 16750623, false, true);

		addText(37469, "100%", fonts, 0, 16750623, true, true);
		addText(37470, "10.5%", fonts, 0, 16750623, true, true);
		addText(37471, "90 H 45 M 30 S", fonts, 0, 16750623, false, true);

		addText(37472, "", fonts, 0, 16750623, false, true);
		addText(37473, "", fonts, 0, 16750623, false, true);
		addClickableText(37474, "", "Select", fonts, 0, 16750623, true, true, 150);
		addClickableText(37475, "", "Select", fonts, 0, 16750623, true, true  , 150);

		addHoverButtonWSpriteLoader(37476, 1264, 40, 40, "", -1, 37477, 1);
		addHoveredImageWSpriteLoader(37477, 1264, 40, 40, 37478);

		addHoverButtonWSpriteLoader(37479, 1264, 40, 40, "", -1, 37480, 1);
		addHoveredImageWSpriteLoader(37480, 1264, 40, 40, 37481);

		addHoverButtonWSpriteLoader(37482, 1264, 40, 40, "", -1, 37483, 1);
		addHoveredImageWSpriteLoader(37483, 1264, 40, 40, 37484);

		addHoverButtonWSpriteLoader(37485, 1264, 40, 40, "", -1, 37486, 1);
		addHoveredImageWSpriteLoader(37486, 1264, 40, 40, 37487);


		tab.totalChildren(30);
		tab.child(0, 26620, 0, 0);
		tab.child(1, 26603, 3, 0);
		tab.child(2, 26604, 0, 245);
		tab.child(3, 26609, 35, 249);
		tab.child(4, 26610, 55, 2);
		tab.child(5, 26611, 55, 2);
		tab.child(6, 26613, 101, 2);
		tab.child(7, 26614, 101, 2);
		tab.child(8, 26616, 9, 2);
		tab.child(9, 26617, 9, 2);
		tab.child(10, 26619, 9, 29);
		tab.child(11, 37463, 12, 53);
		tab.child(12, 37464, 50, 89);
		tab.child(13, 37465, 140, 89);
		tab.child(14, 37466, 90, 117);
		tab.child(15, 37467, 30, 140);
		tab.child(16, 37468, 100, 140);
		tab.child(17, 37469, 55, 155);
		tab.child(18, 37470, 145, 155);
		tab.child(19, 37471, 83, 175);

		tab.child(20, 37472, 40, 198);
		tab.child(21, 37473, 40, 224);

		tab.child(22, 37476, 6, 193);
		tab.child(23, 37477, 6, 193);

		tab.child(24, 37479, 52, 193);
		tab.child(25, 37480, 52, 193);

		tab.child(26, 37482, 52+46, 193);
		tab.child(27, 37483, 52+46, 193);

		tab.child(28, 37485, 52+46+46, 193);
		tab.child(29, 37486, 52+46+46, 193);
	}


	private static void dps(){
		RSInterface test123 = addTabInterface(53321);

		addText(53322, "0 Dps", fonts, 0, 16750623, false, true);

		test123.totalChildren(1);
		test123.child(0,53322,150,150);
	...
	private static void questTabInterface2() {
		int interfaceId = 50850;
		RSInterface tab = addTabInterface(50850);
		interfaceId++;
		addSpriteLoader(26602, 722);
		addSpriteLoader(26603, 1327);
		addSpriteLoader(50851, 1334);
		addSpriteLoader(26604, 722);
		...
		addHoverButtonWSpriteLoader(50852, 1333, 21, 18, "", -1, 50853, 1);
		addHoveredImageWSpriteLoader(50853, 1336, 21, 18, 50854);
		interfaceId+= 3;
		m = interfaceId+1;
		l = interfaceId+2;
		k = interfaceId+3;

		addHoverButtonWSpriteLoader(50855, 1329, 21, 18, "", -1, 50856, 1);
		addHoveredImageWSpriteLoader(50856, 1338, 21, 18, 50857);
		interfaceId+= 3;
		m = interfaceId+1;
		l = interfaceId+2;
		k = interfaceId+3;
		addHoverButtonWSpriteLoader(50858, 1330, 21, 18, "", -1, 50859, 1);
		addHoveredImageWSpriteLoader(50859, 1337, 21, 18, 50860);
		interfaceId+= 3;

		addText(50861, "Server Summary", fonts, 2, 16750623, false, true);

		addText(50862, "Server time: 343434", fonts, 1, 16750623, false, true);
		addText(50863, "Players online: 54", fonts, 1, 16750623, false, true);
		addText(50864, "Events", fonts, 3, 16750623, false, true);
		addText(50865, "Well:", fonts, 0, 16750623, false, true);
		addText(50866, "Eternal:", fonts, 0, 16750623, false, true);
		addText(50867, "Eternal:", fonts, 0, 16750623, false, true);
		addText(50868, "Eternal:", fonts, 0, 16750623, false, true);
		addText(50869, "Eternal:", fonts, 0, 16750623, false, true);
		addText(50870, "Eternal:", fonts, 0, 16750623, false, true);
		addText(50871, "Eternal:", fonts, 0, 16750623, false, true);

		interfaceId = 50851;
		tab.totalChildren(20);
		tab.child(0, 50851, 0, 0);
		tab.child(1, 26603, 3, 0);
		...
		tab.child(4, 50852, 55, 2);
		interfaceId++;

		tab.child(5, 50853, 55, 2);
		interfaceId++;
		interfaceId++;

		tab.child(6, 50855, 101, 2);
		interfaceId++;

		tab.child(7, 50856, 101, 2);
		interfaceId++;
		interfaceId++;

		tab.child(8, 50858, 9, 2);
		interfaceId++;
		tab.child(9, 50859, 9, 2);
		interfaceId++;
		interfaceId++;
		tab.child(10, 50861, 9, 29);
		tab.child(11, 50862, 12, 79);
		tab.child(12, 50863, 12, 54);
		tab.child(13, 50865, 12, 106);
		tab.child(14, 50864, 65, 125);
		tab.child(15, 50866, 12, 125+17);
		tab.child(16, 50867, 12, 125+17+17);
		tab.child(17, 50868, 12, 125+17+17+17);
		tab.child(18, 50869, 12, 125+17+17+17+17);
		tab.child(19, 50870, 12, 125+17+17+17+17+17);
	...
	private static void questTabInterface3() {
		int interfaceId = 50650;
		RSInterface tab = addTabInterface(50650);
		RSInterface scroll = addTabInterface(50690);

		interfaceId++;
		addSpriteLoader(26602, 722);
		addSpriteLoader(26603, 1327);
		addSpriteLoader(50651, 1335);
		addSpriteLoader(26604, 722);
		...
		addHoverButtonWSpriteLoader(50652, 1329, 21, 18, "", -1, 50653, 1);
		addHoveredImageWSpriteLoader(50653, 1338, 21, 18, 50654);
		interfaceId+= 3;
		m = interfaceId+1;
		l = interfaceId+2;
		k = interfaceId+3;

		addHoverButtonWSpriteLoader(50655, 1331, 21, 18, "", -1, 50656, 1);
		addHoveredImageWSpriteLoader(50656, 1337, 21, 18, 50657);
		interfaceId+= 3;
		m = interfaceId+1;
		l = interfaceId+2;
		k = interfaceId+3;
		addHoverButtonWSpriteLoader(50658, 1332, 21, 18, "", -1, 50659, 1);
		addHoveredImageWSpriteLoader(50659, 1336, 21, 18, 50660);
		interfaceId+= 3;
		...
		tab.child(4, 50652, 55, 2);
		interfaceId++;

		tab.child(5, 50653, 55, 2);
		interfaceId++;
		interfaceId++;

		tab.child(6, 50655, 101, 2);
		interfaceId++;

		tab.child(7, 50656, 101, 2);
		interfaceId++;
		interfaceId++;

		tab.child(8, 50658, 9, 2);
		interfaceId++;
		tab.child(9, 50659, 9, 2);
		interfaceId++;
		interfaceId++;


		tab.child(10, 50690, 0, 56);
		tab.child(11, 50661, 9, 29);


//
		addText(50661, "Staff panel", fonts, 2, 16750623, false, true);
			addText(50663, "Main Owners", fonts, 1, 16750623, false, true);
		addText(50664, "King", fonts, 0, 16750623, false, true);
		addText(50665, "Viper", fonts, 0, 16750623, false, true);

		//develop
		addText(50666, "Developers", fonts, 1, 16750623, false, true);
		addText(50667, "Bunny", fonts, 0, 16750623, false, true);
		addText(50668, "N/A", fonts, 0, 16750623, false, true);

		//admin
		addText(50669, "Administrator", fonts, 1, 16750623, false, true);
		addText(50670, "Bunny", fonts, 0, 16750623, false, true);
		addText(50671, "N/A", fonts, 0, 16750623, false, true);

		//admin
		addText(50672, "Moderators", fonts, 1, 0x87857e, false, true);
		addText(50673, "Bunny", fonts, 0, 16750623, false, true);
		addText(50674, "N/A", fonts, 0, 16750623, false, true);

		//admin
		addText(50675, "Supporters", fonts, 1, 16750623, false, true);
		addText(50676, "Bunny", fonts, 0, 16750623, false, true);
		addText(50677, "N/A", fonts, 0, 16750623, false, true);



		//		addText(50669, "Mods", fonts, 1, 16750623, false, true);
//		addText(50670, "Bunny", fonts, 0, 16750623, false, true);
//		addText(50671, "N/A", fonts, 0, 16750623, false, true);
//		addText(50672, "Supporters", fonts, 1, 16750623, false, true);
//		addText(50673, "Bunny", fonts, 0, 16750623, false, true);
//		addText(50674, "N/A", fonts, 0, 16750623, false, true);
		scroll.width = 166; scroll.height = 162; scroll.scrollMax = 835;


		scroll.totalChildren(15);

		scroll.child(0, 50663, 11, 1);
		scroll.child(1, 50664, 11, 1+18);
		scroll.child(2, 50665, 11, 1+18+18);
		scroll.child(3, 50666, 11, 1+18+18+18);
		scroll.child(4, 50667, 11, 1+18+18+18+18);
		scroll.child(5, 50668, 11, 1+18+18+18+18+18);
		scroll.child(6, 50669, 11, 1+18+18+18+18+18+18);
		scroll.child(7, 50670, 11, 1+18+18+18+18+18+18+18);
		scroll.child(8, 50671, 11, 1+18+18+18+18+18+18+18+18);
		scroll.child(9, 50672, 11, 1+18+18+18+18+18+18+18+18+18);
		scroll.child(10, 50673, 11, 1+18+18+18+18+18+18+18+18+18+18);
		scroll.child(11, 50674, 11, 1+18+18+18+18+18+18+18+18+18+18+18);
		scroll.child(12, 50675, 11, 1+18+18+18+18+18+18+18+18+18+18+18+18);
		scroll.child(13, 50676, 11, 1+18+18+18+18+18+18+18+18+18+18+18+18+18);
		scroll.child(14, 50677, 11, 1+18+18+18+18+18+18+18+18+18+18+18+18+18+18);


//		scroll.child(7, 50669, 11, 53+13+13+13+13+13+13);
//		scroll.child(8, 50670, 11, 53+13+13+13+13+13+13+13+2);
//		scroll.child(9, 50671, 11, 53+13+13+13+13+13+13+13+13+2);
//		scroll.child(10, 50672, 11, 53+13+13+13+13+13+13+13+13+13);
//		scroll.child(11, 50673, 11, 53+13+13+13+13+13+13+13+13+13+13+2);
//		scroll.child(12, 50674, 11, 53+13+13+13+13+13+13+13+13+13+13+13+2);
	}
	...
	private static void effectInterface(TextDrawingArea[] tda) {//wtf did u do with it lol and why are there several, it should all be in 1 interface
		...
		addSpriteLoader(48301, 1305);
		addText(48302, "000 Minutes", tda, 0, ColorConstants.ORANGE, false, true);
		addText(48303, "God mode", tda, 0, ColorConstants.SNOW_WHITE, false, true);
	...
	private static void effectInterface3(TextDrawingArea[] tda) {
		...
		addSpriteLoader(48501, 1320);
		addText(48502, "(::opensale)", tda, 0, ColorConstants.ORANGE, false, true);
		addText(48503, "Active Sale", tda, 0, ColorConstants.YELLOW, false, true);
	...
	public static void addNpc(int ID, int npcId) {
		...
		petCanvas.disabledAnimationId = -1;
		petCanvas.enabledAnimationId = -1;
		petCanvas.mediaID = npcId;
		petCanvas.mediaType = 5;
	}
	

	private static void dropTableCheckerInterface(TextDrawingArea[] rsFont) {
		final RSInterface main_widget = addTabInterface(37600);
		String dir = "BossDrops/BACKGROUND";

		setChildren(19, main_widget);

		addSpriteLoader(37601, 1326);
		setBounds(37601, 8, 7, 0, main_widget);

		addText(37602, "NPC Drop Table Checker", 0xFF981F, true, true, -1, 2);
		setBounds(37602, 270, 15, 1, main_widget);

		addHoverButtonWSpriteLoader(37603, 1015, 21, 21, "Close", -1, 37604, 3);
		addHoveredImageWSpriteLoader(37604, 1016, 21, 21, 2);

		setBounds(37603, 483, 14, 2, main_widget);
		setBounds(37604, 483, 14, 3, main_widget);

		addHoverButtonWSpriteLoader(37605, 1323, 100, 32, "Search", 0, 37606, 1);
		setBounds(37605, 25,40, 4, main_widget);
		addHoveredImageWSpriteLoader(37606, 1324, 100, 32, 36706 + 1);
		setBounds(37606, 25,40, 5, main_widget);

		addHoverButtonWSpriteLoader(37608, 1323, 100, 32, "Search", 0, 37609, 1);
		setBounds(37608, 25,68, 6, main_widget);
		addHoveredImageWSpriteLoader(37609, 1324, 100, 32, 36709 + 1);
		setBounds(37609, 25,68, 7, main_widget);

		addSpriteLoader(37610, 1325);

		setBounds(37610, 32, 48, 8, main_widget);
		setBounds(37610, 32, 76, 9, main_widget);

		addText(37611, "Search Item", 0xFF981F, false, true, -1, 0);
		setBounds(37611, 55, 50, 10, main_widget);
		addText(37612, "Search NPC", 0xFF981F, false, true, -1, 0);
		setBounds(37612, 55, 80, 11, main_widget);


		// clickable text widget
		setBounds(37650, 2, 105, 12, main_widget);

		// item widget
		setBounds(37900, 111-13, 65, 13, main_widget);

		addText(37901, "Item", 0xFF981F, true, true, -1, 2);
		addText(37902, "Amount", 0xFF981F, true, true, -1, 2);
		addText(37903, "Rarity", 0xFF981F, true, true, -1, 2);

		setBounds(37901, 215, 43, 14, main_widget);
		setBounds(37902, 331, 43, 15, main_widget);
		setBounds(37903, 434, 43, 16, main_widget);
		setBounds(54503, 2, 2, 17, main_widget);

		addText(37904, "@yel@This only display the base drop rate, meaning it doesn't account for drop boosts", 0xFF981F, true, true, -1, 1);
		setBounds(37904, 258, 305, 18, main_widget);


		final RSInterface clickable_text_widget = addTabInterface(37650);
		clickable_text_widget.height = 193;
		clickable_text_widget.width = 114;
		...
		addToItemGroup(37915, 1, 70, 0, 1, false, null, null, null);
		...
		addSpriteLoader(38553, 1321);
		addSpriteLoader(38554, 1322);

		for (int index = 0; index < 70; index++) {
			final int childId = startIndex + index;
			setBounds(index % 2 == 0 ? 38553 : 38554, 44, yPosition, 0 + index, item_widget);
			yPosition += 33;
		}
		...
		for (int index = 0; index < 70; index++) {
			final int childId = startIndex + index;
			addText((40_500+index), "", 0xFFFFFF, true, true, -1, 0);

			setBounds(40_500+index, 135, yPosition, 70 + index, item_widget);
			yPosition += 33;
		}
		yPosition = 5;
		for (int index = 0; index < 70; index++) {
			final int childId = startIndex + index;
			addText((40_500+index+70), "", 0xFFFFFF, true, true, -1, 0);

			setBounds(40_500+index+70, 235, yPosition, 140 + index, item_widget);
			yPosition += 33;
		}
		...
		for (int index = 0; index < 70; index++) {
			final int childId = startIndex + index;
			addText((40_500+index+140), "", 0xFFFFFF, true, true, -1, 0);

			setBounds(40_500+index+140, 335, yPosition, 210 + index, item_widget);
			yPosition += 33;
		}
		setBounds(37915, 48, -1, 280, item_widget);
	   ...
	   private static void customCombiner(TextDrawingArea[] tda) {
	        ...
	        addSpriteLoader(STARTING_POINT + 1, 1309);
	        addHoverButtonWSpriteLoader(STARTING_POINT + 2, 1310, 127, 34, "Invent", -1, STARTING_POINT + 3, 1);
	        addHoveredImageWSpriteLoader(STARTING_POINT + 3, 1311, 127, 34, STARTING_POINT + 4);
	        ...
	        main.child(4, STARTING_POINT + 6, 324, 210);//boot it up
	        ...
	        addToItemGroup(STARTING_POINT + 21, 4, 3, 5, 7, true, new String[]{"Select", null, null, null, null});
	    }
		protected static void combiner(TextDrawingArea[] tda) {
			...
			addText(19703, "Grand Max items", tda, 0, ColorConstants.ORANGE, true, true);

			addText(19704, "Grand Max Requirements", tda, 0, ColorConstants.ORANGE, true, true);
			...
			addClickableText(19706, "Invent Item", "Invent Item", tda, 1, ColorConstants.ORANGE, true, true, 100);
			addClickableText(19707, "Invent Item", "Invent Item", tda, 1, ColorConstants.ORANGE, true, true, 100);
			addClickableText(19708, "Invent Item", "Invent Item", tda, 1, ColorConstants.ORANGE, true, true, 100);
			addClickableText(19709, "Invent Item", "Invent Item", tda, 1, ColorConstants.ORANGE, true, true, 100);
	...
	public static void gamblingInterface(TextDrawingArea[] tda) {
		...
		addToggleButton(57190, 1077, 1700, 119, 15, "Flower Poker");
		addToggleButton(57191, 1077, 1701, 119, 15, "Dice Duel(ft3)");
		...
		addToggleButton(57194, 1077, 1702, 119, 15, "Blackjack");
		addText(57195, "Blackjack (playername hosts)", ColorConstants.WHITE, false, true, 52, tda, 0);
		addToggleButton(57196, 1077, 1703, 119, 15, "Blackjack");
	...
	public static void dailyRewards(TextDrawingArea[] tda) {
		...
		addSpriteLoader(57211, 1260);


		int j = 57224;
		for(int i = 0; i < 10; i++) {
			addText((j+i), "Day "+(i+1), 0xff9933, true, true, 52, tda, 1);
		}

		addText(57234, "@yel@Daily reward ", 0xff9933, true, true, 52, tda, 2);
		addText(57235, "Time remaining:  ", 0xff9933, false, true, 52, tda, 2);
		addText(57236, "@yel@Claim", 0xff9933, false, true, 52, tda, 2);

		addHoverButtonWSpriteLoader(57237, 1091, 72, 24, "", -1, 57238, 1);
		addHoveredImageWSpriteLoader(57238, 1092, 72, 24, 57239);

//		addSpriteLoader(57219, 1209);
...
//
		addToItemGroup(57223, 5, 2, 38, 63, true, new String[] { null, null, null, null, null });
//
//		addHoverText(57224, "@or2@Claim Day 1", "Claim", tda, 1, ColorConstants.ORANGE, false, true, 100, 14);
//
//		addHoverText(57225, "@or2@Claim Day 2", "Claim", tda, 1, ColorConstants.ORANGE, false, true, 100, 14);
//
//		addHoverText(57226, "@or2@Claim Day 3", "Claim", tda, 1, ColorConstants.ORANGE, false, true, 100, 14);
//
//		addHoverText(57227, "@or2@Claim Day 4", "Claim", tda, 1, ColorConstants.ORANGE, false, true, 100, 14);
//
//		addHoverText(57228, "@or2@Claim Day 5", "Claim", tda, 1, ColorConstants.ORANGE, false, true, 100, 14);
//
//		addHoverText(57229, "@or2@Claim Day 6", "Claim", tda, 1, ColorConstants.ORANGE, false, true, 100, 14);
//
...
//		addConfigButtonWSpriteLoader(57239, 57210, 1210, 1211, 119, 21, "", 1, 0, 1810);
//		addText(57240, "Vote", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57241, 57210, 1210, 1211, 119, 21, "", 1, 0, 1811);
//
//		addText(57242, "Login", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57243, 57210, 1210, 1211, 119, 21, "", 1, 0, 1812);
//		addText(57244, "Vote", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57245, 57210, 1210, 1211, 119, 21, "", 1, 0, 1813);
//
//		addText(57246, "Login", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57247, 57210, 1210, 1211, 119, 21, "", 1, 0, 1814);
//		addText(57248, "Vote", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57249, 57210, 1210, 1211, 119, 21, "", 1, 0, 1815);
//
//		addText(57250, "Login", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57251, 57210, 1210, 1211, 119, 21, "", 1, 0, 1816);
//		addText(57252, "Vote", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57253, 57210, 1210, 1211, 119, 21, "", 1, 0, 1817);
//
//		addText(57254, "Login", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57255, 57210, 1210, 1211, 119, 21, "", 1, 0, 1818);
//		addText(57256, "Vote", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57257, 57210, 1210, 1211, 119, 21, "", 1, 0, 1819);
//
//		addText(57258, "Login", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57259, 57210, 1210, 1211, 119, 21, "", 1, 0, 1820);
//		addText(57260, "Vote", ColorConstants.ORANGE, false, true, 52, tda, 0);
//		addConfigButtonWSpriteLoader(57261, 57210, 1210, 1211, 119, 21, "", 1, 0, 1821);
//
//		addHoverText(57262, "Claim Day 7", "Claim", tda, 0, ColorConstants.ORANGE, false, true, 100, 14);
//
		main.totalChildren(19);
		main.child(0, 57211, 70, 35);
//57223

		main.child(1, 57223, 97, 98+15);
		main.child(2, 57224, 115+1, 62+15);
		main.child(3, 57225, 115+70, 62+15);
		main.child(4, 57226, 115+73+68, 62+15);
		main.child(5, 57227, 115+73+73+65, 62+15);
		main.child(6, 57228, 115+73+73+73+61, 62+15);

		main.child(7, 57229, 115+1, 62+95+15);
		main.child(8, 57230, 115+70, 62+95+15);
		main.child(9, 57231, 115+73+68, 62+95+15);
		main.child(10, 57232, 115+73+73+65, 62+95+15);
		main.child(11, 57233, 115+73+73+73+61, 62+95+15);
		main.child(12, 45830, 411, 41);
		main.child(13, 45831, 411, 41);
		main.child(14, 57234, 115+73+65, 62-18);

		main.child(15, 57235, 85+1, 62+95+15+101);
		main.child(16, 57237, 92+1+150+115, 62+95+15+99);
		main.child(17, 57238, 92+1+150+115, 62+95+15+99);
		main.child(18, 57236, 92+1+150+135, 62+95+15+101);
...
//		main.child(51, 57262, 285, 245);
	...
	private static void startInterface(TextDrawingArea[] tda) { // thats java conventions
		...
		addSpriteLoader(48551, 1342);
		addText(48552, "Welcome Starter Interface", tda, 2, ColorConstants.AQUA, false, true);
		addText(48553, "Starter Hub", tda, 2, ColorConstants.AQUA, false, true);
		addText(48554, "These are quick teleports and -", tda, 0, ColorConstants.AQUA, false, true);
		addText(48555, "Guides to help you out. We strive ", tda, 0, ColorConstants.AQUA, false, true);
		addText(48556, "To make it better for new players.", tda, 0, ColorConstants.AQUA, false, true);
		addText(48557, "If you need more assistance dont", tda, 0, ColorConstants.AQUA, false, true);
		addText(48558, "Hesitate to ask the community,", tda, 0, ColorConstants.AQUA, false, true);
		addText(48559, "We are a very friendly server.", tda, 0, ColorConstants.AQUA, false, true);

		addHoverButtonWSpriteLoader(48560, 1343, 133, 23, "Teleport", -1, 48561, 1);
		addHoveredImageWSpriteLoader(48561, 1344, 133, 23, 48562);
		addText(48563, "Starter Progression", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48564, 1343, 133, 23, "Teleport", -1, 48565, 1);
		addHoveredImageWSpriteLoader(48565, 1344, 133, 23, 48566);
		addText(48567, "Shops", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48568, 1343, 133, 23, "Teleport", -1, 48569, 1);
		addHoveredImageWSpriteLoader(48569, 1344, 133, 23, 48570);
		addText(48571, "Drop Table", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48572, 1343, 133, 23, "Teleport", -1, 48573, 1);
		addHoveredImageWSpriteLoader(48573, 1344, 133, 23, 48574);
		addText(48575, "Upgrade Items", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48576, 1343, 133, 23, "Teleport", -1, 48577, 1);
		addHoveredImageWSpriteLoader(48577, 1344, 133, 23, 48578);
		addText(48579, "Auction House", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48580, 1343, 133, 23, "Teleport", -1, 48581, 1);
		addHoveredImageWSpriteLoader(48581, 1344, 133, 23, 48582);
		addText(48583, "Donation Deals", tda, 0, ColorConstants.YELLOW, false, true);

		//
		addHoverButtonWSpriteLoader(48584, 1345, 76, 32, "Open Link", -1, 48585, 1);
		addHoveredImageWSpriteLoader(48585, 1346, 76, 32, 48586);
		addText(48587, "Starter guide", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48588, 1345, 76, 32, "Open Link", -1, 48589, 1);
		addHoveredImageWSpriteLoader(48589, 1346, 76, 32, 48590);
		addText(48591, "Price guide", tda, 0, ColorConstants.YELLOW, false, true);

		addHoverButtonWSpriteLoader(48592, 1345, 76, 32, "Open Link", -1, 48593, 1);
		addHoveredImageWSpriteLoader(48593, 1346, 76, 32, 48594);
		addText(48595, "Slayer Guide", tda, 0, ColorConstants.YELLOW, false, true);

		addText(48596, "- Quick Teleports -", tda, 2, ColorConstants.AQUA, false, true);
		addText(48597, "- Useful Links -", tda, 2, ColorConstants.AQUA, false, true);
		addButtonWSpriteLoader(48598, 737, "Close Window", 16, 16);
	...
	public static void GoldenScratchCardNew(TextDrawingArea[] tda) {
		...
		RSInterface.addText(39302, "ImaginePS Golden Scratch Card", tda, 2, 0xFF9900, true, true); // 20014
	...
	private static void BoxRewardList(TextDrawingArea[] textDrawingAreas) {
		...
		addSprite(17551, 0, "RewardList/Background");

		RSInterface.addText(17552,  "ImaginePS Reward List", textDrawingAreas, 2,
				0xEAC117, true);
		...
		addHoveredButton(17557, dir, 2, 16, 16, 17558);
		...
		for (int i = 0; i < npcList; i++) {
			RSInterface.addHoverText(17651 + i, "", "View Reward", textDrawingAreas, 0, 0xFF9900, false, true, 150);
		}
		...
		interfaces.child(9, 17558, 50 + xOffset, 40 + yOffset);
	   ...
	   public static void pvmInterface(TextDrawingArea[] tda) {
			final int ID = 13400;
			//Main Interface ID, and calling the addinterface method
			RSInterface tab = RSInterface.addInterface(ID);

			//Main Interface Sprites
			//addSprite(ID + 1, 0, "Interfaces/teleport/BG");
			addSpriteLoader(ID + 1, 1043);

			//Interface Text
			RSInterface.addText(ID + 2, "Preview", tda, 2, 0xFF9900, true, true);
			RSInterface.addText(ID + 3, "Description", tda, 2, 0xFF9900, true, true);
			RSInterface.addText(ID + 4, "Rare Drops", tda, 2, 0xFF9900, true, true);
			RSInterface.addText(ID + 5, "NPC Names", tda, 2, 0xFF9900, true, true);

			//monster preview
			addPet(ID + 6, 100);

			// names scroll
			RSInterface scrollInterface = addTabInterface(ID + 7);
			scrollInterface.width = 105;
			scrollInterface.height = 255;
			scrollInterface.scrollMax = 1330;
			setChildren(100, scrollInterface);
			int y = 5;
			for (int i = 0; i < 100; i++) {
				addHoverText(ID + 8 + i, "", "Select", tda, 2, 0xff9633, false, true, 100, 14);
				setBounds(ID + 8 + i, 0, y, i, scrollInterface);
				y += 15;
			}

			// new id after adding the scroll with 100 capacity
			final int ID2 = ID + 108; // ID = 13400  ID2 = 13508

			// npc category/type buttons!
			addHoverText(ID2, "@cya@Monsters", "@cya@Monsters", tda, 2, 0xff9633, false, true, 100, 14);
			addHoverText(ID2 + 1, "@cya@Bosses", "@cya@Bosses", tda, 2, 0xff9633, false, true, 100, 14);
			addHoverText(ID2 + 2, "@cya@Minigame", "@cya@Minigame", tda, 2, 0xff9633, false, true, 100, 14);
			addHoverText(ID2 + 3, "@cya@Global", "@cya@Global", tda, 2, 0xff9633, false, true, 100, 14);
			addHoverText(ID2 + 16, "@cya@Ultra Bosses", "@cya@Ultra Bosses", tda, 2, 0xff9633, false, true, 100, 14);
			// close button
			addHoverButton(ID2 + 4, CLOSE_BUTTON, CLOSE_BUTTON, 16, 16, "Close", 0,
					ID2 + 5, 3);
			addHoveredButton(ID2 + 5, CLOSE_BUTTON_HOVER, CLOSE_BUTTON_HOVER, 16, 16,
					ID2 + 6);

			// teleport button
			addHover(ID2 + 6, 1, -1, ID2 + 7, 0, "Interfaces/teleport/button", 132, 23, "Teleport");
			addHovered(ID2 + 7, 1, "Interfaces/teleport/button", 132, 23, ID2 + 8);

			// teleport text
			addText(ID2 + 8, "@or2@Teleport", fonts, 2, 0x1aaf0f, false, true);

			// description
			addText(ID2 + 9, "Name:", fonts, 2, 0xff9633, false, true);
			addText(ID2 + 10, "Hitpoints:", fonts, 2, 0xff9633, false, true);
			addText(ID2 + 11, "Attack style:", fonts, 2, 0xff9633, false, true);
			addText(ID2 + 12, "Tier:", fonts, 2, 0xff9633, false, true);
			addText(ID2 + 200, "Recom. gear:", fonts, 2, 0xff9633, false, true);
	        addText(ID2 + 201, "Drop rate:", fonts, 2, 0xff9633, false, true);

			// drops item group, finally! xD
			//itemGroup(ID2 + 14, 8, 2, 8, 3);

			// should add a scroll for the drops item group

			//drops scroll interface!
			RSInterface dropScroll = addTabInterface(ID2 + 13);
			dropScroll.width = 320;
			dropScroll.height = 58;
			dropScroll.scrollMax = 400;
			setChildren(1, dropScroll);
			itemGroup(ID2 + 15, 8, 4, 8, 8);
			dropScroll.child(0, ID2 + 15, 0, 0);

			// teleport button
			addHovered(ID2 + 203, 1, "Interfaces/teleport/button", 132, 23, ID2 + 204);
			// try-it text

			int frame = 0;
			tab.totalChildren(26);
			tab.child(frame, ID + 1, 4, 7); frame++;
			tab.child(frame, ID + 2, 212, 65); frame++;
			tab.child(frame, ID + 3, 402, 65); frame++;
			tab.child(frame, ID + 4, 223, 241); frame++;
			tab.child(frame, ID + 5, 82, 47); frame++;
			tab.child(frame, ID + 6, 150, 100); frame++;
			tab.child(frame, ID + 7, 15, 68); frame++;
			tab.child(frame, ID2, 25, 18); frame++;
			tab.child(frame, ID2 + 1, 110, 18); frame++;
			tab.child(frame, ID2 + 2, 185, 18); frame++;
			tab.child(frame, ID2 + 3, 275, 18); frame++;
			tab.child(frame, ID2 + 16, 333, 18); frame++;
			tab.child(frame, ID2 + 4, 485, 11); frame++;
			tab.child(frame, ID2 + 5, 485, 11); frame++;
			tab.child(frame, ID2 + 6, 155, 204); frame++;
			tab.child(frame, ID2 + 7, 193, 205); frame++;
			tab.child(frame, ID2 + 8, 193, 205); frame++;
			tab.child(frame, ID2 + 9, 336, 86); frame++;
			tab.child(frame, ID2 + 10, 336, 102); frame++;
			tab.child(frame, ID2 + 11, 336, 118); frame++;
			tab.child(frame, ID2 + 12, 336, 134); frame++; // 19
			tab.child(frame, ID2 + 13, 160, 265); frame++;
			tab.child(frame, ID2 + 200, 336, 150); frame++;
	        tab.child(frame, ID2 + 201, 336, 166); frame++;
			tab.child(frame, ID2 + 202, 330, 204); frame++;
			tab.child(frame, ID2 + 203, 330, 204); frame++;
		}

		public static void addPet(int ID, int NpcID) {
			RSInterface petCanvas = interfaceCache[ID] = new RSInterface();
			petCanvas.id = ID;
			petCanvas.parentID = ID;
			petCanvas.type = 6;
			petCanvas.atActionType = 0; //brb
			petCanvas.contentType = 3291;
			petCanvas.width = 136;
			petCanvas.height = 168;
			petCanvas.transparancy = 0;
		...
		public static void drawNpcOnInterface(int childId, int npcId, int zoom) {
			...
			rsInterface.modelRotation1 = 150; //these should be right, didnt cross reference but yh
			rsInterface.modelRotation2 = 0;
			rsInterface.npcDisplay = rsInterface.npcDisplay = NPCDef.forID(npcId).standAnim;
		}
		
		private static void mysteryBoxViewer(TextDrawingArea[] tda) { // lol nice sprites xD
			RSInterface main = addInterface(48030);
			addSpriteLoader(48031, 1306);
			addText(48032, "Common Rewards (55%)", tda, 0, 0x92d397, true, true);
			addText(48033, "Uncommon Rewards (35%)", tda, 0, 0xd3c692, true, true);
			addText(48034, "Rare Rewards (5%)", tda, 0, 0xbb92d3, true, true);
			addText(48035, "Mbox Name", tda, 1, 0xff8624, false, true);
			
			addHoverButtonWSpriteLoader(48036, 1308, 90, 25, "Open", -1, 48037, 1);
			addHoveredImageWSpriteLoader(48037, 1307, 90, 25, 48038);
			
			addHoverButtonWSpriteLoader(48039, 1308, 90, 25, "Open All", -1, 48040, 1);
			addHoveredImageWSpriteLoader(48040, 1307, 90, 25, 48041);
			
			addText(48042, "Open All ", tda, 0, 0xff8624, true, true);
			addText(48043, "Open All to Bank", tda, 0, 0xff8624, true, true);
			addButtonWSpriteLoader(48044, 34, "Close Window", 16, 16);
	...
	public static void mysteryBox(TextDrawingArea[] tda) {
		...
		addSpriteLoader(id, 1038);
		tab.child(c++, id++, 0 + x, 0 + y);
		addText(id, "Box Interface", tda, 2, 0xFFA500, true, true);
		tab.child(c++, id++, 240 + x, 3 + y);

		tab.child(c++, 47300, 116 + x, 77 + y);

		addText(id, "Possible loot:", tda, 1, 0xFFA500, true, true);
		tab.child(c++, id++, 170 + x, 60 + y);

		addText(id, "", tda, 1, 0xFFA500, true, true);
		tab.child(c++, id++, 392 + x, 85 + y);

		addText(id, "", tda, 1, 0xFFA500, true, true);
		tab.child(c++, id++, 393 + x, 115 + y);

		id++;

		addHoverButtonWSpriteLoader(id, 810, 72, 32, "Open", 0, id + 1, 1);
		tab.child(c++, id++, 164 + x, 216 + y);
		addHoveredImageWSpriteLoader(id, 811, 72, 32, id + 1);
		tab.child(c++, id++, 164 + x, 216 + y);

		id++;
		addText(id, "Open", tda, 2, 0xFFA500, true, true);
		tab.child(c++, id++, 189+10 + x, 223 + y);

		id++;

		addHoverButtonWSpriteLoader(id, 810, 72, 32, "Open all", 0, id + 1, 1);
		tab.child(c++, id++, 284 + x, 216 + y);
		addHoveredImageWSpriteLoader(id, 811, 72, 32, id + 1);
		tab.child(c++, id++, 284 + x, 216 + y);

		id++;
		addText(id, "Open All", tda, 2, 0xFFA500, true, true);
		tab.child(c++, id++, 289+27 + x, 223 + y);

		id++;
		addHoverButtonWSpriteLoader(id, 1015, 16, 16, "Close Window", 0, id + 1, 1);
		tab.child(c++, id++, 475, 4 + x);
		addHoveredImageWSpriteLoader(id, 1016, 16, 16, id + 1);
		tab.child(c++, id++, 475, 4 + y);

		id++;

		/* Items */
		RSInterface possible = addInterface(47300);
		possible.width = 234;
		possible.height = 100;
		possible.scrollMax = 480;
		possible.totalChildren(81);
		id = 47301;
		c = 0;
		addSpriteLoader(id, 1055);
		...
		possible.child(c++, id++, x, y);
		itemGroup(id, 1, 1, 5, 5);
		possible.child(c++, id++, x += 40, y);
		itemGroup(id, 1, 1, 5, 5);
		possible.child(c++, id++, x += 40, y);
		itemGroup(id, 1, 1, 5, 5);
		possible.child(c++, id++, x += 40, y);
		...
		scroll.width = 474;
		addToItemGroup(47101, 1750, 1, 13, 10, true, "Upgrade", "test", null);
	...
	public static void upgradeInterface(TextDrawingArea[] tda) {
		...
		addSpriteLoader(62201, 1212);
		addButtonWSpriteLoader(62202, 1213, "@or2@Upgrade Items", 241, 30);
		tab.addText(62204, "@or2@ImaginePS Upgrades", fonts, 2, 0xffffff);
		tab.addText(62205, "@or2@Upgradeable Items", fonts, 2, 0xffffff);
		tab.addText(62206, "@or2@Upgrade Items", fonts, 2, 0xffffff);
		tab.addText(62203, "@or2@Coins required: -", fonts, 2, 0xffffff); // 
		tab.addText(62250, "@or2@Upgrade Chance:", fonts, 2, 0x32CD32);
...
// its cuz u have null items. watch. sec weit where do i do so i replaxce bags with coins 1b coins
		addButtonWSpriteLoader(62230, 1214, "@red@Close Interface");
// done. restart both and do ::upgrade apply t command actions to ur object after.
	...
	public static void donationDeals(TextDrawingArea[] tda) {
		...
		addText(57270, "This is only counted from payments, not opening bonds!", ColorConstants.YELLOW, false, true, 52,
				tda, 0);
		addSpriteLoader(57271, 1292);
		addSpriteLoader(57272, 1293);
	...
	public static void Pets(TextDrawingArea atextdrawingarea[])
 {
     ...
     addText(3210, "ImaginePS Point Shop", atextdrawingarea, 3, 0xffcc33, false, true);
     ...
     addText(3276, "Kill ::ixwarriors To Obtain ImaginePS Points!", atextdrawingarea, 3, 0xffcc33, false, true);
	...
	public static void playerOwnedShopInterface(TextDrawingArea[] tda) {
		final String[] options = {"Price", null, null, "Buy", null};
		RSInterface tab = addInterface(32600);
		RSInterface tab2 = addInterface(32620);
		RSInterface tab3 = addInterface(32622);

		addSpriteLoader(32601, 1207);
		addHoverButtonWSpriteLoader(32602, 1282, 150, 35, "Search item/player", -1, 32603, 1);
		addHoveredImageWSpriteLoader(32603, 1283, 150, 35, 32604);
		addHoverSpriteLoaderButton(32606, 1284, 16, 16, "Close", -1,
				32607, 1);
		addHoveredSpriteLoaderButton(32607, 16, 16, 47269, 1285);


		addText(32610, "Viewing player shops", tda, 2, 0xff7000, true, true);
		addText(32611, "Search item/player", tda, 1, 0xff7000, false, true);
		addText(32612, "Collect", tda, 1, 0xff7000, false, true);
		addText(32613, "Manage my shop", tda, 1, 0xff7000, false, true);
		addText(32614, "Refresh all shops", tda, 1, 0xff7000, false, true);




	      addHoverButtonWSpriteLoader(62000, 1289, 99, 19, "Collect 1B Scrolls", -1, 62001, 1);
			addHoveredImageWSpriteLoader(62001, 1290, 99, 19, 62002);
			
			addHoverButtonWSpriteLoader(62003, 1282, 150, 35, "My shop", -1, 62004, 1);
			addHoveredImageWSpriteLoader(62004, 1283, 150, 35, 62005);//wait
			
			addHoverButtonWSpriteLoader(62006, 1282, 150, 35, "Refresh shops", -1, 62007, 1);
			addHoveredImageWSpriteLoader(62007, 1283, 150, 35, 62008);
		int x = 10, y = 10;
		tab.totalChildren(18);
		tab.child(0, 32601, 6, 0+y);
		tab.child(1, 32602, 4+x, 24+y);
		tab.child(2, 32603, 4+x, 24+y);
		//tab.child(2, 32604, 360+x, 273+y);
		tab.child(3, 32606, 463+x, 4+y);
		tab.child(4, 32607, 463+x, 4+y);
		tab.child(5, 32611, 18+x, 33+y);
		//tab.child(6, 32612, 363+x, 276+y);
		tab.child(6, 32620, 160+x, 60+y);
		tab.child(7, 32622, 10+x, 60+y);
		tab.child(8, 32610, 242+x, 5+y);
		tab.child(9, 62000, 385, 278+y);
		tab.child(10, 62001, 385, 278+y);
		tab.child(11, 62003, 164+x, 24+y);
		tab.child(12, 62004, 164+x, 24+y);
		tab.child(13, 62006, 324+x, 24+y);
		tab.child(14, 62007, 324+x, 24+y);
		tab.child(15, 32612, 400+x, 280+y);
		tab.child(16, 32613, 199, 33+y);
		tab.child(17, 32614, 358, 33+y);
		tab2.width = 312;
		tab2.height = 246;
		tab2.scrollMax = 400;
		addToItemGroup(32621, 7, 8, 10, 10, true, options);
		//itemGroup(32621, 7, 8, 10, 10);
		tab2.totalChildren(1);//wait he just pmed me for custom npcs iwanna see what he wanna say
		tab2.child(0, 32621, 14, 14);
		for (int i = 0; i < 100; i++) {
			addClickableText(32623 + i, "  " + i, "Select", tda, 0, 0xcbcbcb, false, true, 632);
		}
		tab3.width = 126;
		tab3.height = 230;//62//46
	...
	public static void playerOwnedShopInterface2(TextDrawingArea[] tda) {
		...
		addSpriteLoader(33601, 1281);

		addHoverButtonWSpriteLoader(33611, 1286, 72, 24, "Go back", -1, 33612, 1);
		addHoveredImageWSpriteLoader(33612, 1287, 72, 24, 33613);
		addText(33614, "Go back", tda, 1, 0xff7000, false, true);
		addHoveredImageWSpriteLoader(33606, 1288, 164, 22, 33607);
		addHoverSpriteLoaderButton(33607, 1284, 16, 16, "Close", -1,
				32608, 1);
		addText(33610, "Viewing my shop", tda, 2, 0xff7000, true, true);
		int x = 10, y = 10;
		tab.totalChildren(8);
		...
		tab.child(5, 33611, 13, 2+y);
		tab.child(6, 33612, 13, 2+y);
		tab.child(7, 33614, 23, 5+y);
		tab2.width = 455;
		tab2.height = 276;
		tab2.scrollMax = 400;
        addToItemGroup(33621, 10, 8, 10, 10, true, new String[] { "Current Price", "Change Price", null, "Withdraw", null });
	...
	static void playerOwnedShopInterface3(TextDrawingArea[] tda) {
		//final String[] options = {"Withdraw 1", "Remove 5", "Remove 10", "Remove All", "Remove X"};
	       RSInterface rsi = addInterface(37000);
	       addSprite(37001, 1289);//need sprites for this one haha
	       addSprite(37002, 1290);//need sprites for this one haha
	       for (int i = 0; i < 28; i++)
	   		//addItemOnInterface(37003 + i, 4393, options);
	...
	public static void newStarter(TextDrawingArea[] tda) {
		...
		addSpriteLoader(52751, 1312);
		addText(52752, "@or2@ImaginePS game mode selection screen", 0xff9633, true, true, 52, tda, 2);
		addText(52753, "Select a game mode", ColorConstants.ORANGE, false, true, 52, tda, 0);
		addText(52754, "Starter pack @red@(Includes: x1 Free Donator mbox) ", ColorConstants.ORANGE, false, true, 52,
				tda, 0);
		addText(52755, "Mode Description:", ColorConstants.ORANGE, false, true, 52, tda, 0);
		addConfigButtonWSpriteLoader(52756, 51750, 1313, 1314, 100, 20, "Select Normal", 0, 5, 1085);
		addConfigButtonWSpriteLoader(52757, 51750, 1313, 1314, 100, 20, "Select Ironman", 1, 5, 1085);
		addConfigButtonWSpriteLoader(52758, 51750, 1313, 1314, 100, 20, "Select Ultimate Ironman", 2, 5, 1085);
		addConfigButtonWSpriteLoader(52773, 51750, 1313, 1314, 100, 20, "Select Veteran Mode", 3, 5, 1085);
		addSpriteLoader(52759, 1319);
		addSpriteLoader(52760, 1318);
		addSpriteLoader(52775, 1317);
		addText(52761, "Normal", 0xff9633, false, true, 52, tda, 0);
		addText(52762, "  Ironman", 0xff9633, false, true, 52, tda, 0);
		addText(52763, "  Ultimate Iron", 0xff9633, false, true, 52, tda, 0);
		addText(52774, "  Comming Soon!", 0xff9633, false, true, 52, tda, 0);
		addText(52764, "Play ImaginePS as an Iron man.", 0xff9633, true, true, 52, tda, 0);
		addText(52765, "You will be restricted from trading, staking and looting items from killed players.", 0xff9933,
				true, true, 52, tda, 0);
		addText(52766, "You will not get a npc drop if another player has done more damage.", 0xff9633, true, true, 52,
				tda, 0);
		addText(52767, "You will have to rely on your starter, skilling, pvming, and shops.", 0xff9633, true, true, 52,
				tda, 0);
		addText(52768, "This game mode is for players that love a challenge.", 0xff9633, true, true, 52, tda, 0);
		addHoverButtonWSpriteLoader(35769, 1310, 123, 30, "Confirm selection", -1, 35770, 1);
		addHoveredImageWSpriteLoader(35770, 1311, 123, 30, 35771);
		addText(52772, "@or2@Confirm", 0xF7AA25, true, true, 52, tda, 2);
		tab.totalChildren(51);
		tab.child(0, 52751, 7, 8);
		tab.child(1, 52752, 250, 17);
		tab.child(2, 52753, 47, 44);
		tab.child(3, 52754, 223, 45);
		tab.child(4, 52755, 120, 228);
		tab.child(5, 52756, 43, 73);
		tab.child(6, 52757, 43, 107);
		tab.child(7, 52758, 43, 142);
		tab.child(8, 52759, 58, 108);
		tab.child(9, 52760, 58, 142);
		tab.child(10, 52761, 65, 76);
		tab.child(11, 52762, 65, 110);
		tab.child(12, 52763, 65, 143);
		tab.child(13, 52764, 162, 247);
		tab.child(14, 52765, 162, 257);
		tab.child(15, 52766, 162, 267);
		tab.child(16, 52767, 162, 277);
		tab.child(17, 52768, 162, 287);
		tab.child(18, 35769, 350, 250);
		tab.child(19, 35770, 350, 250);
		tab.child(20, 52772, 410, 260);
		tab.child(21, 52773, 43, 175);
		tab.child(22, 52774, 64, 177);
		tab.child(23, 52775, 58, 176);
		/**
		 * First row of items
		 */
		int child = 24;
	...
	addHoverButton(37407, "Interfaces/Tele/TAB/TAB", 3, 130, 55, "Donator", -1, 37407, 1);
	addHoverButton(37408, "Interfaces/Tele/TAB/TAB", 4, 130, 55, "MiniGames", -1, 37409, 1);
	addHoverButton(37410, "Interfaces/Tele/TAB/TAB", 5, 130, 55, "Wilderness", -1, 37411, 1);
	...
	addHoveredButton(37437, "Interfaces/Tele/TBUTTONS/BH", 11, 81, 34, 37638);
					...
					addText(50430, "ImaginePS Teleports", fonts, 2, 0xff9040, true, true); //rename it whatever
	...
	addHoveredButton(38433, "Interfaces/Tele/DBUTTONS/BH", 9, 81, 34, 38634);
					...
					addText(50430, "ImaginePS Teleports", fonts, 2, 0xff9040, true, true);
...
public static void editClan(TextDrawingArea[] tda) {
	...
	addHoverSpriteLoaderButton(47267, 737, 16, 16, "Close", -1,
			47268, 1);
	addHoveredSpriteLoaderButton(47268, 16, 16, 47269, 738);//iam pretty sure there was something in player,java aswell not sure
	...
	public static void playersOnline(TextDrawingArea[] tda) {
		...
		addText(57007, "ImaginePS", 0xff9933, true, true, -1, tda, 0);
	...
	/*
		...
		String[] options4 = new String[] {"Absorption", "Save Input", "Anti Aliasing"}
	 */
...
/*
	...
	public static void clanswarsOptions(TextDrawingArea[] tda) {
		...
		addHoverButton(28563, CLOSE_BUTTON, CLOSE_BUTTON, 16, 16, "Close", 250,
				28564, 3);
		addHoveredButton(28564, CLOSE_BUTTON_HOVER, CLOSE_BUTTON_HOVER, 16, 16,
				28565);
	...
	public static void unpack(CacheArchive streamLoader,
		...
		while (stream.currentOffset < stream.buffer.length) {
			...
			if (rsInterface.type == 0) {
				...
				rsInterface.hidden = stream.readUnsignedByte() == 1;
			...
			if (rsInterface.type == 4) {
				rsInterface.message = stream.readString().replaceAll(
						"RuneScape", "ImaginePS");
				if(rsInterface.message.toLowerCase().equals("the bank of ImaginePS")) {
					rsInterface.message = "        The Bank of ImaginePS";
				}
		...
		bankInterface();
		goodiebagInterface(textDrawingAreas);
		prayerTabInterface();
		...
		questTabInterface2();
		questTabInterface3();
		teleportInterfaceNew(textDrawingAreas);
	...
	CustomInterfaces.skillTabInterface();
		...
		/*	CustomInterfaces.prayerTab();
		CustomInterfaces.curseTabInterface();
		CustomInterfaces.quickPrayersInterface();
		CustomInterfaces.quickCursesInterface();
		CustomInterfaces.optionTab();
		CustomInterfaces.bankInterface();
		CustomInterfaces.shopInterface();*/
		/*	CustomInterfaces.pouchCreation();
		CustomInterfaces.summoningTabInterface();
		...
		if (Configuration.findFreeInterfaces) {
			int testValInd = 411;
			for (int i = 0; i < interfaceCache.length; i++) {
				RSInterface a = interfaceCache[i];
				if (a != null && a.valueIndexArray != null) {
					if (a.valueIndexArray[0][1] == testValInd)
						System.out.println("testValInd " + testValInd + " in use");
				}
			}
			...
			//System.out.println(lastUsedInterface + " is the last used interface, interfaceCache.length should be set to " + (lastUsedInterface+1));

			int[] checkIds = new int[] {40500, 40501, 40502, 40503, 40504, 40505, 40506, 40507, 40508, 40600, 40601, 40602, 40603, 40604, 40605, 40606, 40607, 40608};
			for (int i = 0; i < checkIds.length; i++) {
				int checkId = checkIds[i];
				if (interfaceCache[checkId] == null) {
					System.out.println(checkId + " is free");
				} else {
					System.out.println(checkId + " is used");
				}
			}
		}
	}
	...
	public static void examineTab(TextDrawingArea[] tda) {
		...
		addSprite(52101, 0, "Interfaces/ExamineTab/SPRITE");
	...
	public static void achievementsInterface() {
		...
		addText(45001, "ImaginePS Panel", fonts, 2, 16750623, false, true);
		...
		scroll.width = 174; scroll.height = 224; scroll.scrollMax = 2000;//1790
	...
	private static void wildernessInterface() {
		...
		tab.child(4, 25352, 445, 28);
		tab.child(5, 25353, 340, 46);
		tab.child(6, 25354, 445, 46);
		tab.child(7, 25355, 411, 315);
	}	/*
	 * Wilderness interface
	 */
	private static void PNPC() {
		RSInterface tab = RSInterface.addTabInterface(25352);
		tab.totalChildren(5);
		addTransparentSpriteWSpriteLoader(25348, 742, 150);
		addText(25349, "Npc:", fonts, 0, 0xCCCBCB, true, true);
		addText(25350, "None", fonts, 0, 0xCCCBCB, true, true);
		addText(25351, "Time left:", fonts, 0, 0xCCCBCB, false, true);
		addText(25352, "18", fonts, 0, 0xCCCBCB, true, true);
		tab.child(0, 25348, 334, 2);
		tab.child(1, 25349, 364, 10);
		tab.child(2, 25350, 445, 10);
		tab.child(3, 25351, 340, 28);
	...
	private static void emoteTabInterface() {
		...
		int[] ids = { 161, 162, 163, 164, 165, 167, 168, 169, 170, 171, 172,
				173, 19140, 175, 176, 177, 178, 179, 180, 181, 182, 19141, 184,
				185, 186, 187, 666, 667, 6522, 6532, 6533, 6540, 6541, 11101, 11102, 11103 };
		setChildren(ids.length, Interface);
		String[] tooltips = { "Yes", "No", "Bow", "Angry", "Think", "Wave",
				"Shrug", "Cheer", "Beckon", "Laugh", "Jump For Joy", "Yawn",
				"Dance", "Jig", "Spin", "Headbang", "Cry", "Blow Kiss",
				"Panic", "Raspberry", "Clap", "Salute", "Goblin Bow",
				"Goblin Salute", "Glass Box", "Climb Rope", "Lean",
				"Glass Wall", "Zombie Walk", "Zombie Dance", "Zombie Hand",
				"Shield", "Skillcape", "Air Guitar", "Snowman Dance", "Freeze" };
		int[] X = { 8, 47, 92, 138, 7, 46, 90, 134, 5, 49, 91, 137, 4, 48, 88,
				133, 6, 49, 97, 135, 8, 51, 86, 136, 0, 47, 94-3, 139-3, 3, 49, 86,
				130, 3, 49, 89, 130};
		int[] Y = { 6, 5, 5, 6, 56, 54, 55, 54, 104, 104, 104, 104, 153, 153,
				153, 152, 203, 202, 203, 202, 252, 253, 252, 252, 303, 303,
				302, 302, 355, 355, 355, 355, 405, 405, 405, 405};
		int frame = 0;
		for (int i : ids) {
	...
	private static void reportPlayerInterface() {
		...
		int[][] children = { { id + 1, 12, 10 }, { id + 2, 172, 53 },
				{ id + 3, 40, 105 }, { id + 4, 175, 66 },
...
//          
	...
	public static void vidOptions(TextDrawingArea tda[]) {
		...
		tab.child(21, 35560, 40, 163);
		tab.child(22, 35580, 105, 163);
		tab.child(23, 35610, 208, 163);
		tab.child(24, 35640, 310, 163);
	}
	...
	private static void priceCheckerInterface() {
		...
		addText(18248, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(3, 18248, 472, 23);
		...
		addText(18353, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(7, 18353, 120, 85);
		addText(18354, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(8, 18354, 120, 95);
		addText(18355, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(9, 18355, 120, 105);
		addText(18356, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(10, 18356, 190, 85);
		addText(18357, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(11, 18357, 190, 95);
		addText(18358, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(12, 18358, 190, 105);
		addText(18359, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(13, 18359, 260, 85);
		addText(18360, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(14, 18360, 260, 95);
		addText(18361, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(15, 18361, 260, 105);
		addText(18362, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(16, 18362, 330, 85);
		addText(18363, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(17, 18363, 330, 95);
		addText(18364, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(18, 18364, 330, 105);
		addText(18365, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(19, 18365, 400, 85);
		addText(18366, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(20, 18366, 400, 95);
		addText(18367, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(21, 18367, 400, 105);
		addText(18368, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(22, 18368, 120, 145);
		addText(18369, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(23, 18369, 120, 155);
		addText(18370, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(24, 18370, 120, 165);
		addText(18371, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(25, 18371, 190, 145);
		addText(18372, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(26, 18372, 190, 155);
		addText(18373, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(27, 18373, 190, 165);
		addText(18374, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(28, 18374, 260, 145);
		addText(18375, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(29, 18375, 260, 155);
		addText(18376, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(30, 18376, 260, 165);
		addText(18377, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(31, 18377, 330, 145);
		addText(18378, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(32, 18378, 330, 155);
		addText(18379, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(33, 18379, 330, 165);
		addText(18380, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(34, 18380, 400, 145);
		addText(18381, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(35, 18381, 400, 155);
		addText(18382, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(36, 18382, 400, 165);
		addText(18383, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(37, 18383, 120, 205);
		addText(18384, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(38, 18384, 120, 215);
		addText(18385, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(39, 18385, 120, 225);
		addText(18386, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(40, 18386, 190, 205);
		addText(18387, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(41, 18387, 190, 215);
		addText(18388, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(42, 18388, 190, 225);
		addText(18389, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(43, 18389, 260, 205);
		addText(18390, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(44, 18390, 260, 215);
		addText(18391, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(45, 18391, 260, 225);
		addText(18392, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(46, 18392, 330, 205);
		addText(18393, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(47, 18393, 330, 215);
		...
		addText(18401, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(55, 18401, 190, 260);
		addText(18402, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(56, 18402, 190, 270);
		addText(18403, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(57, 18403, 190, 280);
		addText(18404, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(58, 18404, 260, 260);
		addText(18405, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(59, 18405, 260, 270);
		addText(18406, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(60, 18406, 260, 280);
		addText(18407, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(61, 18407, 330, 260);
		addText(18408, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(62, 18408, 330, 270);
		addText(18409, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(63, 18409, 330, 280);
		addText(18410, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(64, 18410, 400, 260);
		addText(18411, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(65, 18411, 400, 270);
		addText(18412, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(66, 18412, 400, 280);
		addText(18413, "", fonts, 0, 0xFFFFFF, true, true);rsi.child(67, 18413, 260, 155);
		/**
		 * First row of items
		 */
		int child = 68;
...
//			}
	...
	public static void addScroll(int id, int height, int width,
		...
		for (int i = id + 20; i < id + 20 + ACHIEVEMENTS; i++) {
			...
			frame++;
				...
				public static void addScrollHoverText(int id, int height, int width,
						 int scrollAmount, int amount, TextDrawingArea[] tda) {
			final RSInterface scroll = addInterface(id);
			final int ACHIEVEMENTS = amount;
			scroll.width = width;
			scroll.height = height;
			...
			int frame = 0;
			scroll.totalChildren(ACHIEVEMENTS);
			for (int i = id; i < id + ACHIEVEMENTS; i++) {
			addHoverText(i, "" + i + "", "Select", tda, 1,0xff9633, false, true, 100, 14);
			scroll.child(frame, i, x, y);
			y += 15;
			frame++;
			}
    ...
    public static void statistics1(final TextDrawingArea[] tda) {
        ...
        addText(id + 3, "ImaginePS", 0xAF6A1B, false, true, 0, tda, 2);
...
//		addText(39155, "ValacialX.net", fonts, 2, 16750623, false, true);
...
*/
	 ...
	 private static void goodiebagInterface(TextDrawingArea[] tda) {
	        final int STARTING_POINT = 49200;
	        RSInterface main = addInterface(STARTING_POINT);

	        final int ROWS = 5;//lemme get sprites brb
	        final int COLUMNS = 4;
	        addSpriteLoader(STARTING_POINT + 1, 1276);
	        addText(STARTING_POINT + 2, "ImaginePS Goodie Box @whi@(@mag@Bfg-9000 GoodieBox)", tda, 2, ColorConstants.ORANGE, false, true);
	        addText(STARTING_POINT + 3, "Select a number\\nand then click\\n on @whi@Claim prize!", tda, 2, ColorConstants.ORANGE, false, true);
	        addHoverButtonWSpriteLoader(STARTING_POINT + 4, 1278, 88, 36, "Claim Prize!", -1, STARTING_POINT + 5, 1);
	        addHoveredImageWSpriteLoader(STARTING_POINT + 5, 1279, 88, 36, STARTING_POINT + 6);
	        ...
	        line.width = 105;
	        line.height = 1;
	        line.disabledColor = ColorConstants.YELLOW;
	        ...
	        for (int y = 0; y < ROWS; y++) {
	            ...
	            for (int x = 0; x < COLUMNS; x++) {
	                int id = x + y * COLUMNS;
	                addButtonWSpriteLoader(STARTING_POINT + 11 + id, 1277, "Choose");
...
public static void addHoverSpriteLoaderButton(int i, int spriteId,
	...
	private static Set<Integer> prestigeSkills = new HashSet<>(Arrays.asList(0, 1, 3, 6, 9, 12, 15, 21));
	...
	public static void skillTabInterface2() {
		...
		skill.children(icons.length + (text.length * 2) + hovers.length + buttons.length * 5 + 1 - 17);
	...
	public static void dropPreview(TextDrawingArea[] tda) {
		...
		addItemOnInterface(47910, 8888, new String[] {"Information"});
		addItemOnInterface(47909, 8888, new String[] {"Information"});
		addItemOnInterface(47918, 8888, new String[] {"Information"});
		addSpriteLoader(47908, 960);
		//addTransparentSpriteWSpriteLoader(74501, 970, 130);
		addText(47902, "Your item:", 0xff9933, true, true, -1, tda, 2);
		//addText(74503, "Current Defender: Rune", 0xff9933, false, true, -1, tda, 0);
		addText(47904, "", 0xff9933, false, true, -1, tda, 0);
		//addTransparentSpriteWSpriteLoader(74501, 970, 130);
		addText(47916, "Reward", 0xff9933, true, true, -1, tda, 2);
		//addText(74503, "Current Defender: Rune", 0xff9933, false, true, -1, tda, 0);
		addText(47917, "", 0xff9933, false, true, -1, tda, 0);
		addText(47919, "Requirements", 0xff9933, true, true, -1, tda, 2);

		addText(47927, "Upgrade x1", 0xff0000, true, true, -1, tda, 2);
		addText(47928, "Upgrade x1", 0xff9333, true, true, -1, tda, 2);
		addText(47929, "Upgrade x1", 0xff9966, true, true, -1, tda, 2);

		addHoverButtonWSpriteLoader(47930, 996, 16, 16, "Close Window", -1, 47931, 1);
		addHoveredImageWSpriteLoader(47931, 997, 16, 16, 49116);


		addHoverButtonWSpriteLoader(47921, 727, 150, 35, "Upgrade", -1, 47922, 1);
		addHoveredImageWSpriteLoader(47922, 726, 150, 35, 5073);

		addHoverButtonWSpriteLoader(47923, 727, 150, 35, "Upgrade 5x", -1, 47924, 1);
		addHoveredImageWSpriteLoader(47924, 726, 150, 35, 5073);

		addHoverButtonWSpriteLoader(47925, 727, 150, 35, "Upgrade all", -1, 47926, 1);
		addHoveredImageWSpriteLoader(47926, 726, 150, 35, 5073);
		...
		tab.child(0, 47908, 10, 5);
		tab.child(1, 47902, 75+10, 250-200);
		tab.child(2, 47900, 61+10, 292-200);
		tab.child(3, 47904, 25+10, 270-200);

		//text
		tab.child(4, 47909, 61+80, 292-125);
		tab.child(5, 47910, 61+105, 292-125);

		//end result
		tab.child(6, 47916, 75+190, 250-200);
		tab.child(7, 47918, 61+190, 292-200);
		tab.child(8, 47917, 15+190, 270-200);
		tab.child(9, 47919, 61+110, 270-130);
		//buttons
		tab.child(10, 47921, 61+287, 270-210);
		tab.child(11, 47922, 61+287, 270-210);//buttons

		tab.child(12, 47923, 61+287, 270-170);
		tab.child(13, 47924, 61+287, 270-170);

		tab.child(14, 47925, 61+287, 270-130);
		tab.child(15, 47926, 61+287, 270-130);

		tab.child(16, 47927, 61+365, 270-202);

		tab.child(17, 47928, 61+365, 270-162);
		tab.child(18, 47929, 61+365, 270-122);

		tab.child(19, 47930, 61+428, 11);
		tab.child(20, 47931, 61+428, 11);
	...
	public static void Sidebar0(TextDrawingArea[] tda) {
	 ...
	 private static void customTabInterface() {
		    int STARTING_POINT = 20200;
		    RSInterface tab = addTabInterface(20200);
		    RSInterface scroll = addTabInterface(20201);
		    addSpriteLoader(20203, 722);
		    addSpriteLoader(20204, 723);
		    addSpriteLoader(20205, 722);
		    addText(20206, "@or2@----Rsps Help Tab----", fonts, 2, 16740352, false, true);
		    addSpriteLoader(20207, 722);
		    scroll.totalChildren(32);
		    scroll.width = 174;
		    scroll.height = 224;
		    scroll.scrollMax = 475;
		    tab.totalChildren(4);
		    tab.child(0, 20203, 0, 22);
		    tab.child(1, 20204, 0, 25);
		    tab.child(2, 20205, 0, 249);
		    tab.child(3, 20201, 0, 25);
		    int yPos = 40;
		    scroll.child(30, 20206, 10, 7);
		    scroll.child(31, 20207, 0, 31);
		    int index = 0;
		    String[] categories = { "@or2@-Collection Log-", "@or2@-Drop Table-", "@or2@-Upgrade Items-", "@or2@-Donation Store-", "@or2@-Player Owned Shops-", "@or2@-Player Panel-", "@or2@-Donaton Deals-", "@or2@-Daily Rewards-" };
		    for (int i = 0; i < 30; i += 2) {
		      addButtonWSpriteLoader(20220 + i, 1275, (index >= categories.length) ? "@or2@" : categories[index]);
		      addText(20255 + i, (index >= categories.length) ? "@or2@Comming Soon!" : categories[index], fonts, 1, 16750623, true, true);
		      scroll.child(i, 20220 + i, 21, yPos);
		      scroll.child(i + 1, 20255 + i, 86, yPos + 5);
		      yPos += 50;
		      index++;
		    } 
		  }
	...
	public static void UpgradeTab(TextDrawingArea[] tda) {
	}
	...
	public static void rebuildClanChatList(boolean clickable, String ignore, boolean owner) {
		...
		for (int i = 29344; i <= 29444; i++) {
			if(clickable && RSInterface.interfaceCache[i].message.length() > 0) {
				addClanChatListTextWithOptions(i, RSInterface.interfaceCache[i].message, ignore, owner, RSInterface.fonts, 0, 0xffffff, 200, 11);
			} else {
				addText(i, RSInterface.interfaceCache[i] == null ? ""+i+"" : RSInterface.interfaceCache[i].message, fonts, 0, 0xffffff, false, true);
			}
		}
	...
	public static void addHoverWSpriteLoader(int i, int spriteId, int width, int height, String text, int contentType,
		...
		tab.hoverType = hoverOver;
		tab.advancedSprite = true;
		tab.disabledSprite = spriteId > 0 ? SpriteLoader.sprites[spriteId] : null;
		tab.enabledSprite = spriteId > 0 ? SpriteLoader.sprites[spriteId] : null;
		tab.width = width;
		tab.height = height;
		tab.tooltip = text;
	}
	public static void addHigherButton(int child, int hoverChild, int hoverImageChild) {
		addHoverButtonWSpriteLoader(child, 951, 16, 20, "Higher", 250, hoverChild, 4);
		addHoveredImageWSpriteLoader(hoverChild, 952, 16, 20, hoverImageChild);
	}
	...
	public static void addHoveredImageWSpriteLoaderTrans(int i, int spriteId, int w, int h, int imgInterface,
		...
		tab.width = w;
		tab.height = h;
		addTransparentSpriteWSpriteLoader(imgInterface, spriteId, opacity);
		tab.totalChildren(1);
		tab.child(0, imgInterface, 0, 0);
	}
	...
	private static final int DRAW_REGULAR_MODEL = 1, DRAW_NPC_MODEL = 2,
			...
			private Model getMediaModel(int drawType, int mediaId) {
				if (drawType == 5) {
					Model npcModel = (Model) NPCDef.modelCache.get(mediaId);
					if (npcModel != null)
						return npcModel;
					NPCDef npcDef = NPCDef.forID(mediaId);
					return npcDef.getModel();
				}
		    ...
		    public static void addCloseButtonSmall(final int child, final int hoverChild, final int hoverImageChild) {
		        addHoverButtonWSpriteLoader(child, 740, 16, 16, "Close", 250, hoverChild, 3);
		        addHoveredImageWSpriteLoader(hoverChild, 741, 16, 16, hoverImageChild);
		    }
			...
			private static void skillTabInterface() {
				...
				addSpriteLoaderButtonWithTooltipBox(31151, 250, "View @or2@Raids @lre@Options", 268, 31152, "Raids: 1/1\nCurrent XP: 14000000\nNext Level: 14000000\nRemainder: 14000000", 60, 0);
	    ...
	    "Decline");*/
			 ...
			 */
			  private static void shopInterface(TextDrawingArea[] tda)
			  {
			    ...
			    addText(19679, " ", 16750623, false, true, 52, tda, 1);
			    addText(19680, " ", 12547357, false, true, 52, tda, 1);
			    ...
			    setBounds(3901, 240, 21, 3, Interface);
			    setBounds(19679, 42, 54, 4, Interface);
			    setBounds(19680, 150, 54, 5, Interface);
			    setBounds(19681, 129, 50, 6, Interface);
			    setBounds(47651, 0, 64, 7, Interface);
			    addToItemGroup(3900, 10, 10, 8, 10, true, 
			      new String[] { "Value", "Buy 1", "Buy 5", "Buy 10", "Buy X", "Examine" });
			    




			    RSInterface scroll = addTabInterface(47651);
			    scroll.width = 465;
			    scroll.height = 186;
			    scroll.scrollMax = 1000;
			    setChildren(1, scroll);
			    setBounds(3900, 60, 11, 0, scroll);
			    
			    Interface = addTabInterface(19682);
			    addSprite(19683, 1, "Shop/SHOP");
			    addText(19684, " ", 12547357, false, true, 52, tda, 1);
			    addText(19685, " ", 16750623, false, true, 52, tda, 1);
			...
			public static void levelUpInterfaces() {
				...
				RSInterface cons = addTabInterface(24399);
...
//				addSkillChatSprite(29599, 21); //
			...
			public static void ExpRewardInterface() {
				...
				addButtonWSpriteLoader(38046, 863, "Choose Pvming", 46, 44);
				...
				addButtonWSpriteLoader(38064, 863, "Choose Archaeology", 46, 44);
				addButtonWSpriteLoader(38067, 863, "Choose Invention", 46, 44);
				addButtonWSpriteLoader(38070, 863, "Choose Constitution", 46, 44);
				addButtonWSpriteLoader(38073, 863, "Choose Raid", 46, 44);
				addButtonWSpriteLoader(38076, 863, "Choose Cooking", 46, 44);
				addButtonWSpriteLoader(38079, 863, "Choose Divination", 46, 44);
		...
		tab.tooltip = tooltip;
			...
			public Sprite sprite2;
			  boolean displayAsterisks;
			    ...
			    public int entityAnim = 0;

    public boolean isHidden() {
        return hidden && type == 0;
    }
